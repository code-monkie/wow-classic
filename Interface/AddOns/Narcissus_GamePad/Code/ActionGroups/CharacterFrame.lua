...
function ag:Init()
    self.frames = {
        [1] = Narci_ItemLevelFrame.CenterButton,
    };

    for _, tabButton in ipairs(Narci.AttributeController:GetNavButtons()) do
        table.insert(self.frames, tabButton);
    end

    self.col = 1;   --Left: CharacterFrame, Right: Title, NavBar
    self.row = 0;
    self.maxRow = #self.frames;
end
...
function ag:PlaceButtonNote()
    if self.currentObj then
        if self.row == 1 then
            self.pad1 = addon.GamePadButtonPool:SetupButton("PAD1", self.currentObj.gamepadLabel, AttributeFrame, "LEFT", self.currentObj, "RIGHT", 4, 0);
        else
            self.pad1 = addon.GamePadButtonPool:SetupButton("PAD1", self.currentObj.gamepadLabel or "", AttributeFrame, "CENTER", self.currentObj, "TOP", 0, 5);
        end
    end
end
